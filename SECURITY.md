...
- Protected version tags run a separate Microsoft Artifact Signing job only after both source-security and Windows release gates pass. GitHub OIDC supplies a short-lived Azure identity restricted to the `release` environment and one certificate profile; no PFX, private key, client secret, or long-lived GitHub token exists in the repository.
...
- The signing job has read-only repository permission and uploads only the signed payload. A fresh job with no Azure environment or signing identity independently rebuilds the protected source tag twice, regenerates the manifest from its own reproducible unsigned archive, imports the signed payload as untrusted data, proves Authenticode-only mutation, finalizes the ZIP, attests it, and transfers it through an immutable short-retention workflow artifact. A separate publication job rechecks exact filenames, checksum, source ref/digest, hosted-runner provenance, and signer workflow before creating a GitHub Release.
...
- Locally generated builds remain unsigned and their adjacent SHA-256 sidecars provide integrity, not publisher identity. Only an asset published by the protected GitHub workflow after Microsoft Artifact Signing and GitHub provenance attestation is an official binary release. The Azure Public Trust identity/certificate profile must be activated before the first such release can run.
