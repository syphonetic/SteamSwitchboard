...
- Locally generated builds remain unsigned and their adjacent SHA-256 sidecars provide integrity, not publisher identity. The `v1.0.0` GitHub prerelease is deliberately labelled as an unsigned evaluation candidate and carries the same limitation. Only an asset published by the protected GitHub workflow after SignPath Foundation Authenticode signing and GitHub provenance attestation is a publisher-authenticated production binary. The SignPath open-source project, origin verification, release policy, artifact configuration, and CI submitter must be approved and activated before the first such release can run.
