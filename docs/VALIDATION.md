...
The Release suite contains **96 passing tests** covering:
...
- JSON round trips, schema migration, duplicate/unknown-property rejection, semantic validation, duplicate/null IDs, 4 MiB limit, unknown selections, durable deletion tombstones, and bounded corrupt-state retention;
- transaction rollback when account add/remove persistence fails;
...
- valid individual SteamID64 parsing, invalid cached identity rejection, persona sanitisation, signed-out/unknown fail-closed behaviour, and authoritative active-account matching;
- every launch-policy state, exact process arguments, repeated stable account checks, unknown/mismatched accounts, untrusted executables, and an executable that changes after assessment;
- exact Steam Chat/login navigation routes, host/path/port/userinfo/lookalike rejection, URI length limits, direct-user external-link gating, and visible user-initiated microphone policy;
- privacy-safe log records, bounded rotation, and single-instance mutual exclusion; and
- selection/removal regressions, cleanup marker lifecycle, game filtering, and collection transitions.

The ordinary gate runs signed-source/locked NuGet restore, formatting verification, deterministic Release compilation, warnings-as-errors, .NET security diagnostics as errors, and all tests.
...
- Semgrep community C# + security-audit configuration: **179 rules, 50 files, 0 findings**;
...
- Release and test NuGet lock files: **0 known vulnerabilities**, with repository-signature validation and package-source mapping enabled.
...
- The packaged process remained healthy through startup, then closed normally. Its temporary extraction directory and `%LOCALAPPDATA%\SteamSwitchboard` test data were removed, with zero processes remaining.
- A final screenshot was captured only after initialisation completed and contains no account, QR, conversation, or credential data.
...
Three read-only reviews separately examined browser/session isolation, native/local-data boundaries, and release/supply-chain handling. They found no critical issue. Their converged findings were fixed as follows:
...
- silent profile-deletion failure → persisted tombstone, visible pending state, and startup retry;
- stale `MostRecent` launch authority and assess/start race → authoritative-only ActiveUser, repeated checks, exact process path/session, Authenticode publisher verification, and locked final recheck;
- UNC/reparse/traversal and unbounded Steam metadata → local-path policy plus parser/path/identity limits;
- duplicate state IDs, unbounded JSON/recovery/logs, and non-transactional mutations → semantic limits, retention, rollback, and single-instance execution; and
...
Reviewed every browser event registration/detachment, main/child/document navigation decision, permission kind, external launch path, hidden-profile state, workspace wording, and cleanup failure path. No host bridge, cookie access, unsafe scheme, saved permission, silent deletion, or identity-verification claim remains.
...
Reviewed all filesystem probes before use, Steam signer/process/account checks, stable launch transition, AppID argument handling, VDF/JSON limits, semantic identity validation, mutation rollback, diagnostic content, and recovery retention. Malformed/remote/linked/unknown state fails closed without modifying Steam.
...
Reviewed beginner flow, accessibility labels, unsigned-build messaging, exact version propagation, dependency trust, scanner results, ZIP construction, pre-extraction validation, adversarial fixtures, smoke assertions, cleanup, and documentation. The unsigned-development and web-identity limitations are prominent rather than hidden.
