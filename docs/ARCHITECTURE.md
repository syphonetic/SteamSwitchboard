...
For a protected version tag, CI first proves that two independent unsigned package builds are byte-identical. The second archive is validated and extracted into a private signing tree with a complete path/length/SHA-256 manifest. Microsoft Artifact Signing receives exactly two absolute file paths (`SteamSwitchboard.exe` and `SteamSwitchboard.dll`) through an OIDC-authenticated Azure CLI session. Finalization rejects any added/deleted path or any mutation outside those two files. For each PE file it also recomputes a normalized content hash that excludes only the checksum, certificate-directory fields, up to seven zero alignment bytes, and the appended end-of-file certificate table; code/resource replacement therefore fails even if the replacement has a valid publisher signature. Both binaries must then carry valid timestamped Authenticode signatures from the configured publisher and same leaf certificate before the ZIP/checksum is rebuilt and atomically replaced.

The signing job cannot write repository contents or releases and uploads only the signed payload. A fresh validation runner with no `release` environment and therefore no Azure signing federation independently rebuilds the protected source tag twice, creates its trusted manifest from its own reproducible unsigned archive, imports the signed payload as untrusted input, verifies Authenticode-only mutation, finalizes the ZIP, and creates GitHub provenance attestations. A compromised signer action therefore cannot choose the executable code or non-signable package content that reaches the release. A separate publication job has no Azure identity and verifies the sidecar plus attestation signer workflow, source ref, source digest, and hosted-runner policy before creating the release. The SHA-256 sidecar remains a convenience for end users; the trusted signature and GitHub attestation establish publisher/build identity.
